...
def main(output_file_name):
    ...
    if file_output is True:
        # output to file (this is set to append!)
        file = open(output_file_name + ".txt", "a")
        file.write("Iter\tWon?\tGoal\tSteps\tAll steps\n")
    ...
    while iteration < maxIterations:
        ...
        print("Iteration #{:05d}\tWon: {}\tGoal: {}\tSteps taken: {:04d}\tSteps: {}".format(iteration, m.won,
                                                                                              goal, step, movements))
        if file_output is True:
            file.write("{:05d}\t{}\t{}\t{:04d}\t{}\n".format(iteration, m.won, goal, step, movements))
        ...
        m.reset()
        # Randomly determine which wing of the T the goal should be in.
        if iteration >= (maxIterations/10) * 2:
            if r.random() < 0.5:
                m.goalX = 2
            else:
                m.goalX = 0
...
if __name__ == "__main__":
    for i in range(10):
        number = "{:05d}".format(i)
        main("./output/T_20left_80random-memory/" + number)
